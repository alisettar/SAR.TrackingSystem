...
public static class JsonSeedProcessor
{
    ...
    {
        var jsonData = JsonSerializer.Deserialize<List<VolunteerJsonData>>(jsonContent, new JsonSerializerOptions
        {
        ...
        foreach (var group in uniqueTeamData)
        {
            ...
            var team = new Team(
                code: uniqueCode,
                name: group.Key.Ekip,
                city: string.IsNullOrWhiteSpace(group.Key.Şehir) ? null : group.Key.Şehir
            );
    ...
    private static string GenerateTeamCode(string teamName)
    {
        ...
        var words = teamName.Split(new[] { ' ', '-' }, StringSplitOptions.RemoveEmptyEntries);
        if (words.Length == 1)
