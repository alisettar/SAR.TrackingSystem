    ...
    ILogger<TeamService> logger) : ITeamService
{
    ...
    {
        var response = await _httpClient.GetAsync($"/teams/{teamId}/members?paginationRequest={{\"SearchText\":\"{Uri.EscapeDataString(request.SearchText ?? "")}\",\"Page\":{request.Page},\"PageSize\":{request.PageSize}}}");
        response.EnsureSuccessStatusCode();
    ...
    public async Task<List<TeamMemberViewModel>> GetTeamMembersListAsync(Guid teamId)
    {
        try
        {
            var response = await _httpClient.GetFromJsonAsync<PaginatedResponse<TeamMemberViewModel>>($"/teams/{teamId}/members?paginationRequest={{\"Page\":0,\"PageSize\":1000}}");
            return response?.Items?.ToList() ?? [];
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error getting team members list for team {TeamId}", teamId);
            return [];
        }
    }
    ...
    {
        var response = await _httpClient.PostAsJsonAsync("/teams", model);
