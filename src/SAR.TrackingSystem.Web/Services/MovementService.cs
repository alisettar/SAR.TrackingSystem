...
public class MovementService(
    ...
    ISectorService sectorService) : IMovementService
{
    ...
    public async Task<List<MovementViewModel>> GetRecentMovementsAsync(int count = 5)
    {
        try
        {
            ...
            return [.. apiResponse.Items.Take(count)];
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error getting recent movements");
            return [];
        }
    }
    ...
    public async Task<Guid> CreateMovementAsync(MovementCreateViewModel model)
    {
        ...
        {
            logger.LogError(ex, "HTTP error creating movement");
    ...
    public async Task<Guid> CreateTeamMovementAsync(TeamMovementCreateViewModel model)
    {
        try
        {
            ...
            logger.LogInformation("Found {Count} team members for team {TeamId}", teamMembers.Count, model.TeamId);
            ...
            foreach (var member in teamMembers)
            {
                try
                {
                    logger.LogInformation("Creating movement for member {MemberName} ({MemberId})", member.FullName, member.Id);
                    ...
                    successCount++;
                    logger.LogInformation("Successfully created movement for {MemberName}", member.FullName);
                }
                catch (Exception ex)
                {
                    logger.LogError(ex, "Team movement failed for member {MemberName} ({MemberId}): {Error}", member.FullName, member.Id, ex.Message);
                    errors.Add($"{member.FullName}: {ex.Message}");
                }
            }

            logger.LogInformation("Team movement completed: {Success}/{Total} successful", successCount, teamMembers.Count);

            if (successCount == 0)
                throw new ApplicationException($"Hiçbir üye için hareket kaydı yapılamadı. Hatalar: {string.Join(", ", errors)}");

            if (errors.Any())
                logger.LogWarning("Partial team movement success: {Success}/{Total}. Errors: {Errors}",
        ...
        {
            logger.LogError(ex, "Error creating team movement");
    ...
    private async Task<List<VolunteerViewModel>> GetTeamMembersAsync(Guid teamId)
    {
        try
        {
            ...
            logger.LogInformation("GetTeamMembersAsync: Found {TeamMemberCount} members for team {TeamId} out of {TotalVolunteers} total volunteers",
    ...
    public async Task<bool> DeleteMovementAsync(Guid id)
    {
        ...
        {
            logger.LogError(ex, "Error deleting movement {MovementId}", id);
            return false;
        }
    }
    ...
    public async Task<bool> CreateQuickEntryAsync(string qrId)
    {
        try
        {
            var volunteers = await volunteerService.GetVolunteersAsync(new PaginationRequest(0, 1000, qrId));
        ...
        {
            logger.LogWarning("Business rule validation failed for QR {QRId}: {Error}", qrId, ex.Message);
            return false;
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error creating quick entry for QR {QRId}", qrId);
            throw;
        }
    }
}
